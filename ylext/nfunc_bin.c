...
YLDEFNF(concat_bin, 2, 9999) {
	...
	{ /* Just scope */
		...
		/* parameter check */
		ylelist_foreach(w)
			ylnfcheck_parameter(ylais_type(ylcar(w),
						       ylaif_sym())
					    || ylais_type(ylcar(w),
							  ylaif_bin()));
	}
	...
	ylelist_foreach(e) {
		if (ylais_type(ylcar(e), ylaif_sym())) {
			...
			if (0 > yldynb_append(&b,
					     (void*)ylasym(ylcar(e)).sym,
					     (unsigned int)sz))
				goto bail;
		} else {
...
YLDEFNF(to_bin, 1, 2) {
	...
	if (ylais_type(ylcar(e), ylaif_bin()))
		...
		if (!b)
			ylnfinterp_fail (YLErr_out_of_memory,
					 "Out Of Memory\n");
...
YLDEFNF(bin_to_num, 1, 1) {
	...
	if ((double)n != n)
		ylnfinterp_fail(YLErr_func_invalid_param,
				"Double and long long values mismatch.\n");
