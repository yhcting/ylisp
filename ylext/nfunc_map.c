...
/* atom trie */
struct _amap {
	/* data structure - can be hash or trie or something else */
	void*                  d;
	pthread_rwlock_t       m;
...
_cb_map_print_walk(void* user, const unsigned char* key,
	...
	/* add space and trailing 0 to make buffer string */
	yldynb_append(b, (unsigned char*)" ", sizeof(" "));
	/* keep going */
	return 1;
}
...
_aif_map_to_string(const yle_t* e, char* b, unsigned int sz) {
	...
	if (sz < bw)
		...
		memcpy(b, yldynbstr_string(&dyb), bw);
	yldynb_clean(&dyb);
...
_aif_map_visit(yle_t* e, void* user, int(*cb)(void*, yle_t*)) {
	 ...
	 *   synchronization for value is not this data structure's
	 *   responsibility.
...
_make_map(yletcxt_t* cxt, yle_t* e, yle_t* a, int pcsz,
	...
	if (pcsz > 0) {
		...
		ylelist_foreach(w) {
			...
			key = (unsigned char*)ylasym(ylcaar(w)).sym;
			keysz = (unsigned int)strlen(ylasym(ylcaar(w)).sym);
			if (1 == (*_amapi(r)->insert)(_amapd(r),
						      key,
						      keysz,
						      v))
...
YLDEFNF(map_insert, 2, 3) {
	...
	key = ylasym(ylcadr(e)).sym;
	keysz = strlen(ylasym(ylcadr(e)).sym);
