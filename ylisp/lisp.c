...
_DEFAIF_EQ_START(nfunc) {
	if (ylanfunc(e0).f == ylanfunc(e1).f)
		return 1;
	return 0;
} _DEFAIF_EQ_END
...
_DEFAIF_TO_STRING_START(bin) {
	...
	if (ylabin(e).sz > sz)
		...
		memcpy(b, ylabin(e).d, ylabin(e).sz);
		return ylabin(e).sz;
	}
} _DEFAIF_TO_STRING_END
...
yleclean(yle_t* e) {
	if (yleis_atom(e)) {
		if (ylaif(e)->clean)
			...
			yllogW(
"There is an atom that doesn't support/allow CLEAN!\n"
			       );
	} else {
		/*
		 * Let'a think of memory block M that is taken from pool,
		 *   but not initialized.
		 * Value of M is invald.
		 * But, All cleaned memory block has NULL car and cdr value.
		 * (see bottom of this funcion and ylmp_block().)
		 * If there is interpreting error before M is initialized,
		 *   M is GCed.
		 * In this case, car and cdr of M can be NULL!.
		 * But, case that only one of car and cdr is NULL,
		 *   is unexpected!
...
ylregister_nfunc(unsigned int version,
		 ...
		 /* AIF. can be ylaif_sfunc / ylaif_nfunc */
	...
	if (yldev_ver_major(version) < yldev_ver_major(YLDEV_VERSION)) {
		yllogE("Version of CNF library is lower than ylisp!!\n"
		       "  ylisp[%d.%d], cnf[%d.%d]\n",
		       yldev_ver_major(YLDEV_VERSION),
		       yldev_ver_minor(YLDEV_VERSION),
		       yldev_ver_major(version),
		       yldev_ver_minor(version));
	...
	if (ylaif_nfunc() == aif)
		...
		yllogE(
"Function type should be one of ATOM_NFUNC, or ATOM_RAW_NFUNC"
		       );
		return YLErr_cnf_register;
	}
	...
	if (1 == ylgsym_insert(sym, 0, e) ) {
		 ...
		 * Warning is shown at symlookup.c
		 *   when nfunc or sfunc type symbol is overwritten.
...
_aprint(yldynb_t* b, yle_t* e) {
	...
	if (ylaif(e)->to_string) {
		while (1) {
			cw = ylaif(e)->to_string(e,
						 (char*)yldynbstr_ptr(b),
						 yldynb_freesz(b));
			if (cw >= 0) {
				b->sz += cw;
...
_eprint(yldynb_t* b, yle_t* e, yltrie_t* map) {
	...
	cdr_e = ylcdr(e);
	/*
	 * When cdr/car can be NULL?
	 * Free block in memory pool has NULL car/cdr value.
	 * Tring to print garbage expr. may lead to the case of
	 *   printing free block.
	 * To handle this exceptional case,
	 *   NULL car/cdr should be treated well.
	 */
	if (car_e) {
		if (!yleis_atom(car_e)) {
	...
	/*
	 * At upper node,
	 *   car/cdr of this node are not considered to detect cycle.
	 */
	if (car_e)
		yltrie_delete(map, (unsigned char*)&car_e, sizeof(yle_t*));
	if (cdr_e)
		yltrie_delete(map, (unsigned char*)&cdr_e, sizeof(yle_t*));
...
ylreadv_ptr(const char* sym, void** out) {
	...
	if (!sym || !out)
		return YLErr_invalid_param;

	_YLREADV_PROLOGUE;

	if (!e)
		ret = YLErr_invalid_param;
	else
		*out = ylacd(e);
...
ylinit(ylsys_t* sysv) {
	...
	if (pthread_mutexattr_init(&_mattr))
		ylassert(0);
	if (pthread_mutexattr_settype(&_mattr, PTHREAD_MUTEX_ERRORCHECK))
		ylassert(0);
